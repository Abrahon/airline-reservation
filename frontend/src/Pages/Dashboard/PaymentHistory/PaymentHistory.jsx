  ...
  return (
    <div className="w-full mx-auto p-4">
      ...
      <div className="overflow-x-auto">
          ...
          <tbody>
            {payments.map((pay, index) => (
              <tr key={pay._id} className="text-center hover:bg-gray-50">
                ...
                <td className="p-2 border">{pay.flightTitle}</td>
                <td className="p-2 border">${pay.amount}</td>
                <td className="p-2 border">{pay.transactionId}</td>
                <td className="p-2 border">{new Date(pay.paymentTime).toLocaleString()}</td>
        ...
        {payments.length === 0 && <p className="text-center mt-4">No payments found.</p>}
      </div>
    </div>
  );
};
